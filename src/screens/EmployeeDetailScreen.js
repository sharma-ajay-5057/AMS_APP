...
const EmployeeDetailScreen = () => {
  ...
  const emp_status = useSelector(
    (state) => state.employeeStatusReducer.emp_status
  );
  ...
  useEffect(() => {
    if (emp_status != null) {
      if (emp_status.data.success == true) dispatch(fetchRequestEmployeeData());
      else {
            ...
            if (item.user_role_id == 3)
              return (
                <TouchableOpacity
                  activeOpacity={1}
                  onPress={() =>
                    navigation.navigate("Single Employee", { id: item.id })
                  }
                >
                  <View
                    style={{
                  ...
                  >
                    <View style={{ flex: 0.5 }}>
                      <Image
                        source={{
                          uri: `https://i.pravatar.cc?image=${item.id}`,
                        }}
                        style={{
                          width: wp(15),
                          height: wp(15),
                    ...
                    >
                      ...
                      <Switch
                        value={item.status == 1 ? isSwitchOn : !isSwitchOn}
